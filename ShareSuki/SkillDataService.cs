...
namespace ShareSuki
{
    public class SkillDataService
    {
        ...
        public int AddSkillData(SkillData skillData)
        {
            ...
            // 新規登録時に自動でマッチング通知を送信
            _ = Task.Run(() => SendMatchingNotificationsAsync(newId));
        ...
        private async Task SendMatchingNotificationsAsync(int newUserId)
        {
            try
            {
            ...
            {
                // エラーログ（実際の運用では適切なログ出力を行う）
                Console.WriteLine($"マッチング通知送信エラー: {ex.Message}");
            }
        }
